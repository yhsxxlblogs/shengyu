...
**图标动态特效**（新增）：
- 呼吸动画（iconPulse）：3秒循环，缩放1.02倍+阴影变化
- 浮动动画（logoFloat）：3秒循环，上下浮动8rpx
...
- 微信图标颜色：#4CAF50（中等绿色）
- 眼睛图标颜色：#BBBBBB（淡化灰色）
- 提示消息：统一使用 `icon: 'none'` 纯文字

### 聊天页面优化

**页眉高度**：132rpx（原88rpx的1.5倍）
- 返回箭头：40rpx
- 用户名：36rpx
- 清空按钮：28rpx
...
**问题**：用户点赞后切换或刷新页面，点赞状态消失，但点赞数正确

**原因**：获取帖子列表时没有传递token，后端无法识别用户

**修复**：在 community.vue 和 main.vue 的请求中添加 Authorization header
...
**问题**：微信登录用户在个人主页显示wxid（如 wc_xxxxxx）

**原因**：SQL查询直接返回username字段

**修复**：使用COALESCE优先显示微信昵称
...
**问题**：mysqldump报错，v_post_details视图引用不存在的列

**原因**：posts表结构变更，移除了like_count和comment_count字段，但视图未更新

**修复**：创建fix_views.sql，使用子查询动态计算统计数

### 4. 字段名不匹配

**问题**：前端使用`post.liked`，后端返回`is_liked`，导致点赞状态不显示

**修复**：统一前端使用`post.is_liked`

### 5. visible字段缺失

**问题**：帖子列表查询报错 "Unknown column 'p.visible'"

**修复**：执行fix_posts_visible.sql添加visible字段
